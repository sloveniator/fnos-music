...
const pkg = {
  ...
  changelog: 'FM 电台可用性修复（0028→0029）：(1) 管理后台「音源与代理 → 在线音乐源」的勾选项改为从服务端 registry 下发，补上此前遗漏的「汽水音乐」—— 之前 FM 提示「汽水音乐源未启用，请先在管理后台开启」，而后台里根本没有汽水开关，用户找不到设置入口（新增 GET /admin/api/library/online-sources）。(2) 后台支持 tab 深链（/admin/#sources 直达音源页）。(3) 消费者端 FM 页在汽水源未启用时给出可点提示条（直链后台音源页），并在开台时重拉一次源清单，后台改完开关无需整页刷新。',
  arch: 'x86_64',
