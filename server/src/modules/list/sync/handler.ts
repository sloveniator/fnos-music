...
const handleListAction = async(userName: string, { action, data }: LX.Sync.List.ActionList, isRemote: boolean = true) => {
  console.log('handleListAction', userName, action)
  switch (action) {
    case 'list_data_overwrite':
      await global.event_list.list_data_overwrite(userName, data, isRemote)
      break
    case 'list_create':
      await global.event_list.list_create(userName, data.position, data.listInfos, isRemote)
      break
    case 'list_remove':
      await global.event_list.list_remove(userName, data, isRemote)
      break
    case 'list_update':
      await global.event_list.list_update(userName, data, isRemote)
      break
    case 'list_update_position':
      await global.event_list.list_update_position(userName, data.position, data.ids, isRemote)
      break
    case 'list_music_add':
      await global.event_list.list_music_add(userName, data.id, data.musicInfos, data.addMusicLocationType, isRemote)
      break
    case 'list_music_move':
      await global.event_list.list_music_move(userName, data.fromId, data.toId, data.musicInfos, data.addMusicLocationType, isRemote)
      break
    case 'list_music_remove':
      await global.event_list.list_music_remove(userName, data.listId, data.ids, isRemote)
      break
    case 'list_music_update':
      await global.event_list.list_music_update(userName, data, isRemote)
      break
    case 'list_music_update_position':
      await global.event_list.list_music_update_position(userName, data.listId, data.position, data.ids, isRemote)
      break
    case 'list_music_overwrite':
      await global.event_list.list_music_overwrite(userName, data.listId, data.musicInfos, isRemote)
      break
    case 'list_music_clear':
      await global.event_list.list_music_clear(userName, data, isRemote)
      break
    default:
