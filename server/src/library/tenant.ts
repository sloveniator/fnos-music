...
// ---------------------------------------------------------------------------
// 回收站：列表 / 恢复 / 彻底删除
//   软删除只是把文件搬进曲库根下的 .gusi-trash/（点开头，扫描器不进），数据并没丢；
//   但此前没有自助入口，用户只能去 NAS 上手动翻点目录。这里补齐闭环。
//   安全阀与删除一致：所有路径都必须落在某个扫描目录的 .gusi-trash/ 之内，
//   杜绝 ../ 之类的越权读取或删除。
// ---------------------------------------------------------------------------
...
const trashRoots = (rawUser: string): string[] =>
  loadTenant(rawUser).settings.dirs.filter(d => !!d).map(d => path.join(d, TENANT_TRASH_DIRNAME))

/** 回收站内相对路径 → 绝对路径；不在任何回收站根内则返回 null */
const resolveInTrash = (rawUser: string, relPath: string): string | null => {
  const rel = String(relPath ?? '')
  if (!rel || rel === '.' || path.isAbsolute(rel) || rel.includes('\0')) return null
  if (rel.split(/[\\/]/).some(seg => seg === '..')) return null
  for (const root of trashRoots(rawUser)) {
...
const walkTrash = (root: string, cb: (full: string) => void): void => {
  let entries: any[]
  try { entries = fs.readdirSync(root, { withFileTypes: true }) as any[] } catch { return }
  for (const e of entries) {
    const full = path.join(root, e.name)
    if (e.isDirectory()) walkTrash(full, cb)
    else if (e.isFile()) cb(full)
  }
}

export const listTenantTrash = (rawUser: string): TrashEntry[] => {
...
const pruneEmptyTrashDirs = (rawUser: string): void => {
  for (const root of trashRoots(rawUser)) {
...
export const restoreTenantTrash = (rawUser: string, relPaths: string[]): TrashOpResult => {
  const dirs = loadTenant(rawUser).settings.dirs.filter(d => !!d)
  const result: TrashOpResult = { ok: 0, failed: [] }
  for (const raw of relPaths) {
    const rel = String(raw ?? '')
    const src = resolveInTrash(rawUser, rel)
    ...
    const owner = dirs
      .filter(d => isInside(src, path.join(d, TENANT_TRASH_DIRNAME)))
  ...
  pruneEmptyTrashDirs(rawUser)
  // 搬回来的文件要重新进索引才能在曲库看到；扫描是异步的，前端稍后刷新即可
  if (result.ok) startTenantScan(rawUser)
  return result
}

/** 彻底删除（不可恢复）；relPaths 传 ['*'] 表示清空整个回收站 */
export const purgeTenantTrash = (rawUser: string, relPaths: string[]): TrashOpResult => {
  const result: TrashOpResult = { ok: 0, failed: [] }
  const targets: string[] = []
  if (relPaths.some(x => String(x) === '*')) {
    for (const root of trashRoots(rawUser)) walkTrash(root, (full) => targets.push(full))
  } else {
    for (const raw of relPaths) {
      const rel = String(raw ?? '')
      const p = resolveInTrash(rawUser, rel)
